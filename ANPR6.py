...
def write_csv(results, output_file):
    ...
    header = [
        'frame_nmr', 'car_id', 'vehicle_bbox', 'vehicle_class', 'score', 'vehicle_confidence',
        ...
        for frame_nmr, frame_results in results.items():
            for car_id, car_data in frame_results.items():
                ...
                writer.writerow([
                    ...
                    vehicle['score'],  # Vehicle detection score
...
config = {
    ...
    'detection_threshold': 0.25,  # For vehicle and license plate detection
    ...
    with tf.compat.v1.Session(graph=mmt_graph) as mmt_sess:
        ...
        while True:
            ...
            for obj in tracked_objects:
                ...
                color_results = color_model(frame[ycar1:ycar2, xcar1:xcar2, :])
                ...
                if car_class == 2:  # For cars, combine Make/Model and Color
                    combined_label = f"ID:{car_id} {mmt_label} {color_label} LP:{license_text}"
                else:  # For other vehicle classes, use only YOLO class label
                    combined_label = f"ID:{car_id} {config['vehicle_classes'].get(car_class, 'Unknown')} {color_label} LP:{license_text}"
                ...
                results[frame_nmr][car_id] = {
                    ...
                    'license_plate': {
                        ...
                        'score': round(license_score, 4) if license_score is not None else None or '',

...
def main():
    ...
    # Process video
    cap = cv2.VideoCapture("/content/drive/MyDrive/Unilactic/MMRfiles/Cam3.mp4")
    if not cap.isOpened():
    ...
    out = cv2.VideoWriter('/content/drive/MyDrive/Unilactic/MMRfiles/output/Cam3o.mp4', cv2.VideoWriter_fourcc(*'mp4v'), 30, (frame_width, frame_height))
    ...
    with tf.compat.v1.Session(graph=mmt_graph) as mmt_sess:
        ...
        frame_nmr = 0
        # Dictionary to store previous frame's data for each tracked object
        prev_frame_data = {}
        ...
        while True:
            ...
            for obj in tracked_objects:
                ...
                # Retrieve previous frame's data for this car_id if exists
                prev_data = prev_frame_data.get(car_id, {
                ...
                mmt_label = prev_data['mmt_label']
                mmt_score = prev_data['mmt_score']
                ...
                if car_class == 2:  # Car class
                    ...
                    new_mmt_label = mmt_labels[mmt_class_index]
                    new_mmt_score = np.max(mmt_predictions)

                    # Update only if new confidence is higher
                    if new_mmt_score > (mmt_score or 0):
                        mmt_label = new_mmt_label
                        mmt_score = new_mmt_score
                ...
                for detection in license_plate_detections.boxes.data.tolist():
                    if (len(detection) > 6):
                        x1, y1, x2, y2, id, lp_score_val, zero = detection
                    else:
                        x1, y1, x2, y2, lp_score_val, zero = detection
                    if xcar1 < x1 < xcar2 and ycar1 < y1 < ycar2 and lp_score_val >= config['lp_confidence_threshold']:
                        license_plate_crop = frame[int(y1):int(y2), int(x1):int(x2), :]

                        # Read license plate
                        new_license_text, new_license_score = read_license_plate(license_plate_crop)

                        # Update license text only if new score is higher
                        if (new_license_text and (license_text is None or new_license_score > license_score)):
                            license_text = new_license_text
                            license_score = new_license_score
                            license_detection = detection
                            lp_score = lp_score_val
                # If no new license plate was detected, check previous frame's data
                if license_text is None and car_id in prev_frame_data:
                # Optionally, you could add a time-based or confidence-based decay
                    license_text = prev_frame_data[car_id].get('license_text')
                    license_score = prev_frame_data[car_id].get('license_score', 0)
                ...
                results[frame_nmr][car_id] = {
                    ...
                    'license_plate': {
                        'bbox': [x1, y1, x2, y2] if license_detection else [None, None, None, None],
                        'text': license_text or '',
                        'score': round(license_score, 4) if license_score is not None else None,
                        'lp_score': round(lp_score, 4) if lp_score is not None else None
                    }
                }

                # Store data for next frame
                if license_text:
                    prev_frame_data[car_id] = {
                        'mmt_label': mmt_label,
                        'mmt_score': mmt_score,
                        'license_text': license_text,
                        'license_score': license_score
                    }
                print(type(license_detection))
                print(license_text)
                ...
                text_position = (int(x1 + 5), int(y1 - 5)) if (x1 and y1) else None
                bg_color = (0, 0, 0)
                ...
                if text_position is not None:
                    ...
                    cv2.rectangle(frame, (int(text_position[0] - 5), int(text_position[1] - 15)), (int(text_position[0] + text_size[0] + 5), int(text_position[1] + 5)), bg_color, -1)  # Black background
    ...
    out.release()
    # Write results to CSV
    write_csv(results, '/content/drive/MyDrive/Unilactic/MMRfiles/output/cam3o.csv')

    ...
    with tf.compat.v1.Session(graph=mmt_graph) as mmt_sess:
        ...
        while True:
            ...
            for obj in tracked_objects:
                ...
                new_color_label = color_results[0].names[color_label_index]
                new_color_confidence = color_results[0].probs.top1conf.item()
                
                # Update color only if new confidence is higher
                if new_color_confidence > (prev_data['color_confidence'] or 0):
                    color_label = new_color_label
                    color_confidence = new_color_confidence
                else:
                    color_label = prev_data['color_label']
                    color_confidence = prev_data['color_confidence']

                # Attempt to match license plates
                license_detection = None
                lp_score = None
                license_text = prev_data['license_text']
                license_score = prev_data['license_score']

                for detection in license_plate_detections.boxes.data.tolist():
                    x1, y1, x2, y2, plate_id, lp_score_val, _ = detection
                    if xcar1 < x1 < xcar2 and ycar1 < y1 < ycar2 and lp_score_val >= config['lp_confidence_threshold']:
                        ...
                        if new_license_text and new_license_score > (license_score or 0):
                ...
                text_position = (x1 + 5, y1 - 20) if license_detection is not None else None
                bg_color = (0, 0, 0)
                ...
                if license_text:
                    ...
                    cv2.rectangle(frame, (text_position[0] - 5, text_position[1] + 15),
                               (text_position[0] + text_size[0] + 5, text_position[1] -5), bg_color, -1)  # Black background
